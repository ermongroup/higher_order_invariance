 function paramsp = nnet_train_geo( thresh, runName, runDesc, paramsp, Win, bin, resumeFile, maxepoch, indata, outdata, numchunks, intest, outtest, numchunks_test, layersizes, layertypes, mattype, rms, errtype, hybridmode, weightcost, decay, jacket)
...
for epoch = epoch:maxepoch
    ...
    if lambda < thresh
        if strcmp(layertypes{numlayers}, 'logistic')
            ...
            %[chs2_e, iter2_e] = conjgrad_1(@(V)-computeBV(V), term1(ch1) + term2(ch1), ch2_e, 2, 2, precon, jacket); 
            %ch2_estimate = chs2_e{end};
            %ch2_e = ch2_estimate;
        elseif strcmp(layertypes{numlayers}, 'linear')
            ...
            %[chs2_e, iter2_e] = conjgrad_1(@(V)-computeBV(V), term1(ch1), ch2_e, 2, 2, precon, jacket);
            %ch2_estimate = chs2_e{end};
            %ch2_e = ch2_estimate;
        elseif strcmp(layertypes{numlayers}, 'softmax')
            [chs2, iterses2] = conjgrad_1(@(V)-computeBV(V), term1(ch1) + term2(ch1), ch2, ceil(maxiters), ceil(miniters), precon, jacket);                       
        ...
        chs2 = cell(size(chs1));
        iterses2 = zeros(size(iterses1));
        for t = 1:length(chs2)
            chs2{t} = zeros(size(chs1{t}));            
        end        
    end
    ch2 = chs2{end};
    ch = ch1 - 0.5 * ch2;
    ...
    delta1 = conv(rate)*chs1{j};
    norm1 = norm(delta1);
    delta2 = 0.5*conv(rate^2)*chs2{j};    
    norm2 = norm(delta2);
    
    outputString( ['ng term norm : ' num2str(norm1)] );
    outputString( ['geo term norm : ' num2str(norm2)] );
    ...
    %Parameter update:
    outputString( ['angle between ng and geo : ' num2str(acos(-makeDouble(chs1{j})' * chs2{j} / norm(chs1{j}) / norm(chs2{j})) * 180/pi)]);
    delta = delta1 - delta2;    
    outputString( ['combined term norm : ' num2str(norm(delta))]);
