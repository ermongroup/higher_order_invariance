function paramsp = nnet_train_midpoint_2( thresh, runName, runDesc, paramsp, Win, bin, resumeFile, maxepoch, indata, outdata, numchunks, intest, outtest, numchunks_test, layersizes, layertypes, mattype, rms, errtype, hybridmode, weightcost, decay, jacket)
...
for epoch = epoch:maxepoch
    ...
    % first pass to compute the mid point
    if lambda < thresh
        ...
        % The first maxiter - used to compute the mid point.
        maxiters = 50;
        miniters = 1;
        ...
        [ll, err] = computeLL(paramsp + p / 2, indata, outdata, numchunks);
        for j = (length(chs)-1):-1:1
            [lowll, lowerr] = computeLL(paramsp + chs{j} / 2, indata, outdata, numchunks);
        ...
        [ll_chunk, err_chunk] = computeLL(paramsp + chs{j} / 2, indata, outdata, numchunks, targetchunk);
        ...
        autodamp = 0;
        denom = -0.5*makeDouble(chs{j}'*computeBV(chs{j}/2)/2) - makeDouble(grad'*chs{j}/2);
        autodamp = 1;
        ...
        while j < 60
            ...
            [ll, err] = computeLL(paramsp + conv(rate) / 2 *p, indata, outdata, numchunks);
        end
        ...
        paramsp0 = paramsp;
        paramsp = paramsp + conv(rate) / 2 * p;
        outputString(['mid ng norm : ' num2str(norm(conv(rate)/2*p))]);
    else
        paramsp0 = paramsp;
        rate = 1.0;
    end
    ...
    [ll, err] = computeLL(paramsp0 + conv(rate) * p, indata, outdata, numchunks);
    [oldll, ~] = computeLL(paramsp0, indata, outdata, numchunks);
    for j = (length(chs)-1):-1:1
        [lowll, lowerr] = computeLL(paramsp0 + conv(rate) * chs{j}, indata, outdata, numchunks);
    ...
    p = chs{j};
    [ll_chunk, err_chunk] = computeLL(paramsp0 + conv(rate) * chs{j}, indata, outdata, numchunks, targetchunk);
    [oldll_chunk, olderr_chunk] = computeLL(paramsp0, indata, outdata, numchunks, targetchunk);
    ...
    %or even the whole thing
    autodamp = 0;
    denom = -0.5*makeDouble(conv(rate)*chs{j}'*computeBV(conv(rate)*chs{j})) - makeDouble(grad'*conv(rate)*chs{j});
    autodamp = 1;
    rho = (oldll_chunk - ll_chunk)/denom;
    if oldll_chunk - ll_chunk > 0
        rho = -Inf;
    ...
    [old_ll_c, old_err_c] = computeLL(paramsp0, indata, outdata, numchunks);
    outputString(['old_ll: ' num2str(old_ll_c) ' old_err: ' num2str(old_err_c)])    
    for k = 1:20
        test = randn(size(delta));
        test = test / norm(test) * norm(delta) / 2;
        [ll_c, err_c] = computeLL(paramsp0 + test, indata, outdata, numchunks);
        if ll_c < old_ll_c
    ...
    outputString(['final ng norm : ' num2str(norm(delta))]);
    paramsp = paramsp0 + delta;
    paramsp0 = [];
