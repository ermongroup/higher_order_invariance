...
class GaussianMlpPolicy(object):
    def __init__(self, ob_dim, ac_dim, name='pi'):
        ...
        self.ob_dim = ob_dim
        self.ac_dim = ac_dim
        self.pdtype = pdtype = make_pdtype(gym.spaces.Box(low=-np.ones([ac_dim], dtype=np.float32),
                                                          high=np.ones([ac_dim], dtype=np.float32)))
        ob_no = U.get_placeholder(dtype=tf.float32, shape=[None, ob_dim * 2], name="ob")  # batch of observations
        oldac_na = U.get_placeholder(dtype=tf.float32, shape=[None, ac_dim], name="ac")  # batch of actions previous actions
        oldac_dist = U.get_placeholder(dtype=tf.float32, shape=[None, ac_dim * 2],
                                    name="oldac_dist")  # batch of actions previous action distributions
        adv_n = U.get_placeholder(dtype=tf.float32, shape=[None], name="adv")  # advantage function estimate
        ...
        self.logstd_1a = logstd_1a = tf.expand_dims(logstd_1a, 0)
        self.std_1a = std_1a = tf.exp(logstd_1a)
        std_na = tf.tile(std_1a, [tf.shape(mean_na)[0], 1])
        ac_dist = tf.concat([tf.reshape(mean_na, [-1, ac_dim]), tf.reshape(std_na, [-1, ac_dim])], 1)
        self.pd = pdtype.pdfromflat(ac_dist)
